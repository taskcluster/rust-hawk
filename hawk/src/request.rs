...
impl<'a> Request<'a> {
    ...
    pub fn method(mut self, method: &'a str) -> Self { self.method = method; self }

    /// Set the URL path for the request.
    pub fn path(mut self, path: &'a str) -> Self { self.path = path; self }

    /// Set the URL hostname for the request
    pub fn host(mut self, host: &'a str) -> Self { self.host = host; self }
    ...
    pub fn hash(mut self, hash: Option<&'a [u8]>) -> Self { self.hash = hash; self }

    /// Set the `ext` Hawk property for the request
    pub fn ext(mut self, ext: Option<&'a str>) -> Self { self.ext = ext; self }

    /// Set the `app` Hawk property for the request
    pub fn app(mut self, app: Option<&'a str>) -> Self { self.app = app; self }

    /// Set the `dlg` Hawk property for the request
    pub fn dlg(mut self, dlg: Option<&'a str>) -> Self { self.dlg = dlg; self }

    /// Create a new Header for this request, inventing a new nonce and setting the
    /// timestamp to the current time.
    pub fn generate_header(&self, rng: &rand::SecureRandom, credentials: &Credentials) -> Result<Header, String> {
        let nonce = try!(random_string(rng, 10));
    ...
    /// and nonce.
    pub fn generate_header_full(&self, credentials: &Credentials, ts: time::Timespec, nonce: String) -> Result<Header, String> {
        let hash_vec;
        let hash = match self.hash {
            None => None,
            Some(v) => { hash_vec = v.to_vec(); Some(&hash_vec) },
        };
        let mac = try!(make_mac(
            &credentials.key,
            ts,
            &nonce,
            self.method,
        ...
        Ok(Header::new(
            ...
            nonce,
            mac,
            match self.ext { None => None, Some(v) => Some(v.to_string()) },
            match self.hash { None => None, Some(v) => Some(v.to_vec()) },
            match self.app { None => None, Some(v) => Some(v.to_string()) },
            match self.dlg { None => None, Some(v) => Some(v.to_string()) },
        ))
    }
}
...
mod test {
    ...
    fn test_generate_header_full() {
        ...
        assert_eq!(header, Header{
            ...
            mac: vec![122, 47, 2, 53, 195, 247, 185, 107, 133, 250, 61, 134, 200, 35, 118, 94, 48, 175, 237, 108, 60, 71, 4, 2, 244, 66, 41, 172, 91, 7, 233, 140],
            ext: None,
            hash: None,
            app: None,
            dlg: None,
    ...
    fn test_generate_header_full_with_optional_fields() {
        ...
        assert_eq!(header, Header{
            ...
            ts: Timespec::new(1000, 100),
            nonce: "nonny".to_string(),
            mac: vec![72, 123, 243, 214, 145, 81, 129, 54, 183, 90, 22, 136, 192, 146, 208, 53, 216, 138, 145, 94, 175, 204, 217, 8, 77, 16, 202, 50, 10, 144, 133, 162],
